<?php //msql/connectors_basic
$r=["_menus_"=>['description'],
"conn"=>['link or image'],
"url"=>['[urlÂ§text] apply the URL to the selected text'],
"img"=>['upload image'],
"video"=>['video : youtube, dailymotion, google, ted, livestream, rutube'],
"iframe"=>['iframe'],
"h"=>['big'],
"b"=>['bold'],
"i"=>['italic'],
"u"=>['underline'],
"k"=>['strike'],
"s"=>['small'],
"q"=>['blockquote'],
"list"=>['list (for each line)'],
"nh"=>['footnote'],
"web"=>['Display the description of a web page'],
"twitter"=>['Twitter API:
- [123456789:twitter] returns a twit
- [textÂ§search:twitter] result of a search
- 123456789Â§thread:twitter] thread of a discussion (upwards)'],
"art"=>['Open an article : 
- [1234:art] button to open an article with his title
- [1234Â§title:art] free titre
- [title:art] : find article in current language'],
"msql"=>['return datas of a table : 
[hub_table_(version)-(key)|(row)Â§option:microsql] ;
options : pop, read, conn, last, count, graph, form, tmp'],
"table"=>['- columns : | or comma
- lines : Â¬ or break-line
- headers : Â§1'],
"center"=>['align to center'],
"right"=>['align to right'],
"--"=>['horizontal line'],
"nbsp"=>['indivisible space'],
"quo"=>['quotes'],
"qu"=>['quotes tag'],
"select"=>['select all'],
"copy"=>['copy'],
"paste"=>['paste'],
"deline"=>['reduce new lines'],
"delconn"=>['delete connector'],
"findconn"=>['select connector'],
"del"=>['delete'],
"nl"=>['new line']]; ?>