...
'BLOCK'=>['Call a block of modules.rnThe rstr60 let display the result into the article. In this case the option specify the width of this column and modify the maximum width of the images.','name of the block of modules','',''],
'MENU'=>['Calls a block as a menu of links to modules','Name of the block','',''],
'ARTMOD'=>['Calls a block of modules with an article.rnrstr60 let put it in a little button "linked artocles".rnThat let modules of context "art" to be differents.','Name of the block','','display mode'],
...
'app_popup'=>['open an app in a popup','params : button,type,process,paramrnex: hello,art,auto,(id article)','',''],
'apps'=>['Apps','software buttons','',''],
'archives'=>['navigate in time!','Give a title','',''],
'article'=>['simple article','ID','',''],
'articles'=>['Order of the Api specialised in article scrolls','parameters of the Api: tag:word,nbdays:1,preview:autornrn- cat/tag : specify a category / a tagrn- nocat/notag : exit a category / a tagrn- nbdays : time systemrn- preview : 1, 2, 3, autorn','',''],
...
'book'=>['public books','','',''],
'bridge'=>['make a bridge with anoter philum server','param : server without \'html\', option : ID of article ;rnconsole url : /module/bridge/philum.org/1130','ID of article or url console (/module/bridge/philum.fr/236)',''],
...
'channel'=>['receive the datas from others hubs or Philum sites, using selections','(use space)rnex: \'philum.fr:site philum:hub 236:art CMS:tag 10:last\'rnDefinitions :rn:site : (optionnal) without \'http\';rn:cat : (optionnal) a category;rn:art (illogical with cat) : affiliated articles;rn:last : the N last articles ;rnrnChannel can be used in a connector \':ajax\' (splitted with comas) like here :rn[site.com:site blog:hub :channel|Titre, rnclose|x:ajax]','autorefresh (seconds)',''],
...
'finder'=>['Open Finder','param (folder) : hub/root/dir...rnoption (mode) : rn-0=disk/shared/iconsrn-1=local/global/distantrn-2=virtual/realrn-3=list/panel/flap/icons/icon-diskrn-4=normal/recursive/connrn-5=alonern-6=pictos/mini','options for each param: rn- 0 = disk/shared/iconsrn- 1 = local/global/distantrn- 2 = virtual/realrn- 3 = list/panel/flap/icons/icon-diskrn- 4 = normal/recursive/connrn- 5 = alonern- 6 = pictos/mini',''],
...
'related'=>['Articles \'related by\' and \'related to\'','','',''],
...
'submenus'=>['drop menus','syntax :rneach object is a connector \':link\' (ID, ID|titre, category)rneach line generate a buttonrnthe number of tilds give the depthrnbuttons a top of hierarchy can not be a linkrnrnonern- twornthreern- fourrn-- five','horizontal',''],
...
'taxonomy'=>['article hierarchy','','',''],
'topoart'=>['topology of an article folder','id','',''],
...
'video_viewer'=>['video viewer in ajax','rules (|) :rn- tag, cat, priority rn- tag1|tag2 ou 5-tag1|tag2 (5=tags)rn- priority-2|3|4 ou 11-2|3|4 (11=priority)rn- cat-public : articles in \'public\' ;rn- cat-1 : current category','',''],
