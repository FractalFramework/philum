...
Les modules sont chargés en cascade (comme css) : le de
ier efface le précédent. Les conditions sont des itérations : home/cat/art.
...
[réactualiser:b] : utile après une modification exte
e (dans l\'admin Msql, ou pendant la phase de tests avec le constructeur css ouvert dans une autre fenêtre)
...
'designwidths'=>['La gestion des largeurs permet affecter toutes les classes css conce
ées.
...
car elle n\'est pas conce
ée par les mises à jour.
...
Quelques exemples sont fou
is parmi les connecteurs, templates et modules publics.'],
...
Si la restriction \'user templates\' (55) est activée, la machine ira chercher le template utilisateur puis le public, avant d\'utiliser celui par défaut. Pour éviter qu\'un template public ne supplante celui par défaut, il suffit de sauver ce de
ier pour en faire un template utilisateur.'],
...
'rssurl_1'=>['Renvoie les articles récents des flux rss dont on est sûr de vouloir aspirer tous les articles. Seuls sont conce
és les flux marqué 1 à la colonne \'bot\' de la table \'rssurl\'.
...
Chaque atome est représenté par sa configuration électronique. Le plus souvent le nombre atomique correspond à la position du de
ier électron de cet atome.
...
Politique de confidentialité : les informations obtenues ne peuvent être relayées sans l\'autorisation des personnes conce
ées.
'],
...
'utime'=>['- numeric value (strict time) : choose the unit (xee, year, etc.)
...
'deco'=>['Les décorations peuvent être appelée via des connecteurs simplifiés ou explicites, et des raccourcis.
- :under, :double, :dotted, :dashed, :wavy, :strike, :over, :underover : sont des expressions de :deco.
- :u{clr} est un raccourci de :under
- :bdr{clr} est une expression de :border
- :bkg{clr} = background
- :bdl = :borderline est une expression de border avec attribut bottom']]; ?>