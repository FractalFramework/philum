...
'ARTMOD'=>['Llama a un bloque de módulos junto con un artículo.rnEl rstr60 permite mostrarlo en un pequeño botón llamado "artículos conectados", en lugar de un div en la página. Los módulos del contexto "arte" deben reservarse aquí','','',''],
...
'app_popup'=>['ejecuta una app en un popup','params : botón,tipo,proceso,paramrnex: hola,arte,auto,(id de item)','',''],
'apps'=>['Apps','Las apps son botones de software. Se pueden crear botones, enlaces, menús, html, ajax, situados en el menú admin, en un artículo, abriendo listas desplegables, software, in situ, anidados, vinculados a otros botones, vinculados a iconos, en un popup, o en otro lugar... Estas posibilidades se clasifican por tipo de habilidad y ubicación.rnrnNota: las apps con el mismo nombre sustituyen a las anteriores: para anular una app por defecto, añade la misma y ocúltala rnrnLos contextos: rnmenu: menú Apps en el menú adminrndesk: iconos del escritoriornboot: al iniciar la páginarnhome: menú Phi en el menú adminrnuser: menú user en el menú admin (habilitado por rstr48)','',''],
'archives'=>['navegación temporal','Dar un título','',''],
'article'=>['elemento simple','ID','',''],
'articles'=>['Comando api especializado en scrolls de artículos','parámetros api:rntag=Uno&amp;nbdays:1,preview:auto,limit:10rn- cat/tag: especifica una categoría / etiquetarn- nocat/notag: excluye una categoría / etiquetarn- nbdays: campo de tiemporn- preview: 1, 2, 3, auto','',''],
...
'channel'=>['recibe feeds de otros hubs o sitios Philum, incluyendo criterios de clasificación','(parámetros separados por un espacio)rnEjemplo: \'philum.fr:site philum:hub 236:art CMS:tag 10:last\'rnDefiniciones:rn:site: (opcional) sin el \'http\';rn:cat: (opcional) una categoría;rn:art (ilógico con cat): artículos afiliados;rn:last: los N artículos más recientes;rn..rnEl módulo Channel puede ser llamado desde un conector \':ajax\'; rnejemplo: [site.com:site blog:hub :channel|Title, close|x:ajax]','autorefresh (seconds)',''],
...
'desktop'=>['parámetros del escritorio','','',''],
...
'finder'=>['Opera un Finder','param (path) : hub/root/dir...','opciones para cada parámetro : rn- 0 = disco/compartido/iconosrn- 1 = local/global/distantern- 2 = virtual/realrn- 3 = lista/panel/flap/iconos/icon-discorn- 4 = normal/recursivo/connrn- 5 = solorn- 6 = pictos/mini',''],
...
'related'=>['Artículos «relacionados con» y «vinculados a»','','',''],
...
'submenus'=>['menús desplegables','sintaxis:rncada objeto es un conector \':enlace\' (ID, ID|título, categoría)rncada línea corresponde a un botónrnel número de guiones significa profundidadrnlos botones en la parte superior de una jerarquía no pueden ser enlacesrnrnunorn- dosrn- tresrn- cuatrorn-- cinco','horizontal',''],
'suggest'=>['ofrece a los visitantes una forma de sugerir un artículo a partir de una Url','','',''],
'tag_arts'=>['artículos con etiqueta','especificar la etiqueta de referencia para la clasificación;rnsi es necesario, especificar la clase de etiquetarnej: tag:class','',''],
...
'taxonomy'=>['jerarquía de artículos','','',''],
'topoart'=>['topología de una carpeta de artículos','id','',''],
...
'video_viewer'=>['viewer vídeo en ajax','rÃ¨gles de tri sÃ©parÃ©es par \'|\' :rn- tag, cat, priority rn- tag1|tag2 ou 5-tag1|tag2 (5=tags)rn- priority-2|3|4 ou 11-2|3|4 (11=priority)rn- cat-public : articles dans \'public\' ;rn- cat-1 : catÃ©gorie en cours','',''],
...
'BOOT'=>['Módulos que se inician en el arranque','','',''],
'DESK'=>['Módulos de escritorio','Pueden recibir un módulo MENÚ','','',''],
'ADMIN'=>['Módulos de administración','Pueden recibir un módulo MENÚ','',''],
'PHI'=>['','','',''],
