...
function findfunc(e, o) {
	var t = e.innerHTML;//codev
...
function embedslct(debut, fin, id, act) {
	var id = id ? id : 'txtarea';
...
function mozWrap(opn, clo, id, action, acm) {
	var id = id ? id : 'txtarea';
...
function embed(val, bid, id) {
	...
	//if(!slctd)tog_j(bid+'prg_mc,conns__'+bid+'_'+ajx(val)+'__'+bid+'_'+id);
}
...
function embedart(id, rid) {
	...
	if (!val) embedslct('[', ':art]', rid);
		...
		var slctd = embedslct('[' + val + '|', ':art]', rid);
...
//insert
function insert(text, tg) {
	tg = tg ? tg : 'txtarea'; var txtarea = getbyid(tg);
...
function cmd(e) {
	var k = e.keyCode;//115/s
	//if(getbyid('srch').value='')
	if (k == 110) { ajaxcall('popup', 'bubs,call', ['call', 'addart'], ['txtarea'], 'focus:addsrc'); }
}//n //getbyid('addurl').focus();
//addEvent(document,'keypress',function(){cmd(event)});

//toggles
function SaveBc(val) {
	var dn = val.split('_');//artopen
...
function active_list_bubble(ul) {
	var li = ul.getElementsByTagName("li"); //change
	if (li.length > 0) for (i = 0; i < li.length; i++) {
		var liul = li[i].getElementsByTagName("ul");
		li[i].className = ''; if (liul[0]) { liul[0].style.display = 'none'; }
	}
}
function global_actlistbub() {
	var bubs = getbycss('inline');
	for (var i = 0; i < bubs.length; i++)active_list_bubble(bubs[i]);
}
function findparentbub(ob) {
	var pbub = ob.parentNode; if (pbub == null) return ob;
...
function closechild(li) {
	var ul = li.getElementsByTagName("ul"); //desactive
	for (i = 0; i < ul.length; i++) { ul[i].style.display = 'none'; }
}
function closebub(e) {
	active_list_bubble(e.parentNode); eb = e.parentNode.parentNode;//btn
	if (eb.parentNode) setTimeout(function () { closebub(eb) }, 100); else clbub(0, '');
}
function closepbub(e, id) {
	var pbu = e.parentNode.parentNode; active_list_bubble(pbu);
	var li = pbu.getElementsByTagName("li");//if(id)e.parentNode.id=id;
	for (i = 0; i < li.length; i++) {
		if (li[i] == e.parentNode && li[i].id) li[i].className = 'active';
		else li[i].className = '';
	}
}

//close bub
function clbub(op, bid) {
	var div = getbyid('clbub');
	if (op) {
		addEvent(div, 'mousedown', function () { clbub(0, bid) });
		div.style.width = '100%'; div.style.height = '100%';
	}
	else {
		div.style.width = 0; div.style.height = 0;
		if (bid) Hide(bid); else {
			var bub = getbyid('bub');
...
//close popbub
function clpop(e, id) {
	var n = clp.length; var fix = '';
	if (id) clp[n] = id; else if (n) {
		if (e) var m = mouse(e); else m = { x: 0, y: 0 };
		for (i = 0; i < n; i++) {
			if (clp[i]) {
				var bub = getbyid(clp[i]); if (bub) {
...
//close togbub
function cltog(d) {
	var op = getbyid(d); if (op) var ob = op.parentNode; Remove(d);
...
function togglebub(j) {
	var dn = j.split('_'); var id = dn[2]; j = j;
...
function active_modlist(id, n) {
	var mnu = getbyid(id).getElementsByTagName('a');
	for (i = 0; i < mnu.length; i++) { if (mnu[i].id == 'n' + n) mnu[i].className = 'active'; else mnu[i].className = ''; }
}

function active_divlist(div, n) {
	var mnu = div.getElementsByTagName('a');
	for (i = 0; i < mnu.length; i++) { if (mnu[i].id == 'n' + n) mnu[i].className = 'active'; else mnu[i].className = ''; }
}
...
function tog_j(val, nb, nob) {
	var dn = val.split("_"); dn = undefiner(dn, 7);
...
function bubjs(ob, o) {
	...
	if (pp == null) {
		var pp = document.createElement('div'); pp.id = 'bubjs';
	...
	if (o == 1) {
		pp.style.display = 'inline-block'; pp.style.zIndex += 1; pp.innerHTML = res;
...
function bubj(ob, o) {
	...
	if (pp == null) {
		var pp = document.createElement('div'); pp.id = 'bubj';
		pp.style = 'position:absolute; background:white; border;1px solid silver; padding:1px; font-size:12px;';
		getbyid('popup').appendChild(pp);
	}
	if (o == 1) {
		pp.style.display = 'inline-block'; pp.style.zIndex += 1; SaveJ('bubj_' + j);
...
function log_finger(id) {
	var va = getbyid(id).value;
...
function num_finger(id, mx) {
	var va = getbyid(id).value; var n = va.length;
	if (!Number(va)) va = va.substr(0, n - 1);
	if (n > mx) va = va.substr(0, mx);
	getbyid(id).value = va;
}

function num_mail(id) {
	var v = getbyid(id); var va = v.value;//mk::form
	if (va.indexOf("@") == -1) v.className = 'txtred';
	else if (va.indexOf(".") == -1) v.className = 'txtred'; else v.className = '';
}

function log_goodname(id) {
	let va = log_finger(id);
...
function mem_storage(val) {//tar_var_copy_type
	...
	if (vn[2]) {
		var lc = localStorage[vn[1]];//paste
...
function sav_btn(id, div) {
	var mnu = getbyid(div); var rmn = mnu.getElementsByTagName('a');
...
function delconnx(val) {
	var d = getbyid(val).value;
...
function artslide(e, id) {
	var wpos = window.pageXOffset; var met = getbyid('meta' + id);
...
function autoread(id, rid) {
	doc = getbyid("scrll" + rid);
	scrolltxt(1); timer = setTimeout(function () { autoread(id, rid) }, 100);
}
function scrollt(n, rid) {
	doc = getbyid("scrll" + rid);
...
function jtimbt(id, j, n) {
	var ob = getbyid(id); var a = activeid(id);
...
function poplist(id) {
	...
	for (i = 0; i < div.length; i++) {
		var dv = div[i].childNodes[0]; dv.className = c;
...
function artlive2(div) {
	...
	if (idx == -1 && scrl > pos.y) {
		...
		if (rq.value != 'undefined') {
			ajaxcall(div, 'api,callj', [rq.value, 'to:' + id], [], 'after');
		}
	}
}//addiv()
...
function useslct(e, id) {
	var d = getrange('art' + id);
	if (d.txt) {
		var ex = getbyid('edtrk' + id);
...
function xltags(e, id, cnn) {
	var d = getrange('art' + id);
	if (d.txt) {
		var tg = cnn == 'all' ? 'popup' : 'art' + id; if (cnn == 'all') cnn = '';
