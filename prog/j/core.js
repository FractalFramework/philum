...
function active_list_byhref(c,nm){//menus
var div=getbycss(c); var mnu=div[0].getElementsByTagName('a');
for(i=0;i<mnu.length;i++){var lk=decodeURI(strend(mnu[i].href,'/'));
if(lk==nm){mnu[i].classList.add("active");}
else mnu[i].classList.remove("active");}}
...
getbyid(tar).value=decodeURI(nm);}
