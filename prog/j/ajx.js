...
AJAX.prototype.createReqestObject = function () {
	...
	try { req = new XMLHttpRequest(); }//all
		...
		try { req = new ActiveXObject('Microsoft.XMLHTTP'); }//IE
		catch (error) {
			try { req = new ActiveXObject('Msxml2.XMLHTTP'); }//IE
...
AJAX.prototype.handleResponse = function () {
	...
	if (this.m_Request.readyState == 4) {
		...
		else if (act != 2 && tg != 'popup' && tg != 'bubble' && act != 'after') opac(10, tg);
	}// && act!='u'
...
function hj(o) {
	var com = o.href; var r = com.split('/'); r = undefiner(r, 5); var diez;//unused
...
function mkprm(dn2, dn3) {
	var prm = []; var dna = dn2.split(','); var vl = '';
...
function bjcall(val) {
	if (typeof x != undefined) clearTimeout(x);
...
function waitmsg(div) {
	...
	if (dv != undefined) { dv.innerHTML = loadn; } else {
		popw.innerHTML = loadn;
...
function poph(popu, pageup, thin) {
	if (popu == null) return;
...
function autoscroll(popu, thin) {
	...
	if (p.y + p.h > (ha + sc)) {
		var nh = (ha + sc) - p.y - 20; if (nh > 440) nh = 440; if (nh < 100) nh = 140;
...
function bpos(id, nb, p) {//bubblepop
	...
	if (py + pob.h > ha) {
		bl.style.maxHeight = (ha - py - 60) + 'px';
...
function popub(res, act) {
	...
	if (act > 100) var pos = bpos('bt' + act, 'pop' + nb, 0);
		...
		else if (act.substr(0, 2) == 'bt') {
			var ab = 0; var pos = bpos(act, 'pop' + nb, 1);
...
function bubpos(bub, id, li, liul) {
	var indic = id.substr(0, 1);
	if (indic == 'd') {
		var tw = innerW(); var pos = getPosition(li);
...
function bubble(res, id) {
	...
	if (li.className == 'act') { li.className = ''; closechild(li); }
		...
		closeotherbubs(li.parentNode); active_list_bubble(li.parentNode);
...
function panup(res, id) {
	...
	if (liul[0]) {//second call
		...
		li = li.parentNode; li.innerHTML = res;
...
function togbub(res, id) {
	...
	var e = infixed(div); if (e) {
		var poc = get_dim(e); var nx = bsl;
...
function SaveJtim(j, n) {
	if (typeof x != undefined) clearTimeout(x);
	x = setTimeout(function () { SaveJ(j) }, n ? n : 1000);
}//open bub
function clbubtim(e) {
	if (typeof xc != undefined) clearTimeout(xc);
	xc = setTimeout(function () { closebub(e) }, 10000);
}//close bub
...
function rebound(dn) {
	...
	//else if(dn[3]=='u'){updateurl(dn[4],val.replace('_u_','__'));}//see after
		...
		var da = dn[1].split(','); ajaxcall('', da[0] + ',js', [dn[4], dn[5]], [], 17);
...
function SaveIe() {//addurl_fastmenu
	...
	if (src) ajaxcall('popup', 'sav,batchpreview', [src], [], '');
}//Close('popup');
...
function saveart(id) {
	var ob = getbyid('art' + id); getbyid('edt' + id).innerHTML = '';
...
function sjtime(id, j) {
	var ob = getbyid(id);
	if (ob != null) var old = ob.value; else var old = '';
	setTimeout(function () { sjtimb(old, id, j) }, 1000);
}

function autocomp_call(old, id, cats) {
	vn = cats.split(' '); var cur = getbyid('inp' + id).value;
	if (old == '') { for (i = 0; i < vn.length; i++)getbyid('slct' + normalize(vn[i]) + id).innerHTML = ''; }
	else if (cur != old || cur.length < 2) return;
	else for (i = 0; i < vn.length; i++) {
		var tg = 'slct' + normalize(vn[i]) + id;
