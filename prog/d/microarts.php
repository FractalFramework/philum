...
static function rid($p){return 'mia'.str::normalize($p);}
...
$r=sql('id,day,msg,name','qdi','',$id);
$rb=self::read($r,$p);
return view::call(self::tmp(),$rb);}
...
return $bt.view::batch(self::tmp(),$rb);}

static function call($p,$o,$prm=[]){
$p=$prm[0]??$p; $rid=self::rid($p);
$bt=tagb('h2',$p);
...
$ret=self::call($p,$o); $rid=self::rid($p);
return divd($rid,$bt.$ret);}
}
