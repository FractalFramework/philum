...
static function build($id,$p){$lg='fr';//to
...
static function glossary($d,$lga,$lgb){
$es=['Ummo'];
$fr=['Oummo'];
$en=['Oomo'];
return str_replace($$lga,$$lgb,$d);}

static function redo($id,$lga='fr'){//from
$lgb=sql('lg','qda','v',['id'=>$id]);
if($lgb==$lga)return 'no:'.$lga;
$ida=sql('msg','qdd','v',['ib'=>$id,'val'=>'lang'.$lga]);
if(!$ida)return 'noref:'.$lga; if($ida==$id)return 'no:'.$ida;
$r=sql('ib,name,mail,day,nod,frm,suj,re,lu,img,thm,host,lg','qda','a',$ida);
$lgref=$r['lg']; $lgset=$lgb.'-'.$r['lg'];//to,from
//$suj=trans::call('suj'.$ida,$lgset,2);
$suj=trans::read($r['suj'],$lga,$lgb,'html','suj'.$ida);//html
...
sql::upd('qda',$rb,['id'=>$id]);
if($msg)sql::upd('qdm',['msg'=>$msg],['id'=>$id]);
return 'ok';}
...
//$ret.=lj('txtx',$rid.'_transart,empty___'.$p,'trunc').' ';
...
$ret.=lj('txtx',$rid.'_transart,rediff___'.$p,'rediff').' ';
return $ret;}
