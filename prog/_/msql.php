...
if(is_file($f))include $f; elseif($rh)$r=self::save($dr,$nod,[],$rh,$bak); $m=self::$m;
if(isset($r)){if($u && isset($r[$m]))unset($r[$m]); return $r;}}

static function row($dr,$nod,$in,$o='',$rh=[]){$f=self::url($dr,$nod);//assoc
if(is_file($f))include $f; elseif($rh)self::save($dr,$nod,[]); $m=self::$m; if(!isset($r[$in]))return;
if($o && isset($r[$m]))return array_combine_a($r[$m],$r[$in]);
return $r[$in];}

static function col($dr,$nod,$n=0,$u=1,$rh=[]){$f=self::url($dr,$nod);
if(is_file($f))include $f; elseif($rh)self::save($dr,$nod,[]); $m=self::$m; $rb=[];
...
static function assoc($dr,$nod){$r=self::read($dr,$nod); $m=self::$m; $rb=[]; $rh=$r[$m]??'';
