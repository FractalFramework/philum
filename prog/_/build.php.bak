...
return '<form id="upl'.$id.'" style="display:inline-block" method="post" action="#" enctype="multipart/form-data" name="max_file_size" value="200000" onchange="upload(\''.$id.'\')" accept-charset="utf-8"><label class="uplabel btn"><input type="file" id="upfile'.$id.'" name="upfile'.$id.'" multiple />'.hidden('typ'.$id,$typ).$o.picto('upload').'</label></form>'.btd($id.'up','').btd($id.'prg','');}
