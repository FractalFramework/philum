...
if(auth(5)){$r=msql::row('',nod('css'),$cssn);
	$desgname=$r[0]??'';
...
foreach($r as $k=>$v)if($v)$r[$k]=clrneg($v,0);
return $r;}
...
if($k>0){$sty='color:#'.clrneg($v,1).'; background-color:#'.$v.'; ';
...
$ret.=div($bt,'clrp','colorpick'.$i,'color:#'.clrneg($clrs[$i],1));}
