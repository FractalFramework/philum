...
11=>['balise','balise($b,$r,$t)','$b=balise $r=attributs $t=text :: nattributs_rule : array(\"id\"=>1,5=>\'txtx\')nwhere key=html_attribut=>valuennumerical code of attributs is : narray(1=>\"type\",2=>\"name\",3=>\"id\",4=>\"value\",5=>\"class\",6=>\"size\",7=>\"maxlenght\",8=>\"onKeyPress\",9=>\"cols\",10=>\"rows\",11=>\"wrap\",12=>\"action\",13=>\"method\",14=>\"for\",15=>\"onchange\",16=>\"style\")','html balise from an array of attributes','html'],
12=>['bar','bar($d)','$d=number','a black image of width=20px and height=$d','graphics'],
13=>['batch','batch($r,$func)','$r=array $func=function_name','fill the array with the result of value as param of function $func (maths)n','mecanics'],
14=>['batch_defil','batch_defil($r)','array','string: <option>$k</options>','html'],
15=>['batch_defil_kv','batch_defil_kv($r,$here,$kv)','=array =value of a key of the array $kv=usage of the values of the array :nkk=use and echo k;nkv=use k and echo v;nvv=use and echo v;nvk=use v and echo kn-- used by menuder_form_kv','string: <value>$k</value> <option>$v</options> with a checked value','html'],
...
21=>['call_plug_func','call_func($css,$r,$val)','$r=array of params in values $val=echo :: params of SaveJ are : targetdiv_function_(id)_(x/xx)_val1_val2_val3_val4_miwedvar1|mixedvar2nwhere :ntargetdiv=where send the resultnfunction=a condition in the hangars of conditions ajax.phpn(id)=optionnal, a div where read valuen(x/xx)=x close the popup, xx close the popup after 1 secondnval1-4=strings prepared for ajaxnmixedvar= diferents id where catch values','classical ajax button callin \'SaveJ\'','ajax'],
...
105=>['msql_read','msql_read($dr,$nod,$in)','$directory $nod=table $i=range (optionnal) :: resolve root problems','array of a table nor a part of it nor only the value there is only one range','microbases'],
