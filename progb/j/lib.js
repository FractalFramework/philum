...
else var u='/'+dn[4]+(dn[5]?'/'+slug(dn[5])+(dn[6]?'/'+ajx(dn[6],1):''):''); //pr(u);
var r={u:u,a:a,j:j,k:io,t:''};
if(j!=oldj)window.history.pushState(r,a,u); oldj=j;// && r.j!=event.state.r.j
...
function slug(va){
var arr=[' ','è','é','ê','à','â','ô','ö','û','ü','ù','î','ï'];
var arb=['_','e','e','e','a','a','o','o','u','u','u','i','i'];
for(var i=0;i<arr.length;i++)va=strreplace(arr[i],arb[i],va);
return va;}
