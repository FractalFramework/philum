...
if($p['cmd']=='tracks'){$qdi=db('qdi'); //$sq['and'][]=$qdi.'.re="1"'; //$sq['slct']=[$qda.'.id'];//todo:use datas
	$sq['inner'][]='inner join '.$qdi.' on '.$qdi.'.ib='.$qda.'.id'; $sq['gr']=$qda.'.id';}
elseif($p['cmd']=='trktyp'){$qdi=db('qdi'); $sq['and'][]=$qdi.'.re="3"'; //$sq['slct']=[$qda.'.id'];//todo:use datas
	$sq['inner'][]='inner join '.$qdi.' on '.$qdi.'.ib='.$qda.'.id'; $sq['gr']=$qda.'.id';}
...
if(!empty($sq['gr']))$gr=' group by '.$sq['gr']; else $gr='';
return 'select '.$slct.' from '.$qda.''.$in.' where '.$wh.$gr.$ord;}
...
if($ra['cols']??'')return implode($rt);
elseif($cmd=='panel')return implode('',$rt);
return implode('',$rt);}
...
$nbpg=self::head($ra);
if(!$ret)$ret=noresult();
return $nbpg.div($ret,'grid-sections','apicnt');}//

static function js($rid){if(!$rid)$rid='loadmodart';
return 'addEvent(document,"scroll",function(){artlive2("'.$rid.'")});';}

static function boot($ra){
$ra['rid']='loadmodart';//randid('load');
head::add('jscode',self::js($ra['rid']));
//$jb=head::jscode($js);
return div(self::load($ra),'',$ra['rid']);}
