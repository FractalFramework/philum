...
static function sav($p,$o,$prm){$d=$prm[0]??$p;
if($p)[$dr,$pg,$fc]=explode('|',$p);
$f=($dr?$dr:'progb').'/'.$pg; 
if($fc)return self::func_sav($p,'',$prm);
$d='<'.'?php'."\n".$d."\n".'?'.'>'; 
if($_SESSION['auth']>6){write_file($f,$d);
return div('saved','frame-green');}}

static function home($p,$o,$fc=''){
if(!$p)$p='progb'; $dr=$p.'/'; $oa=$o.'.php'; $d='';
...
$ret.=lj('popsav','cbk_codev,sav_codxt_xd_'.$p.'|'.ajx($oa).'|'.ajx($fc),$sav).span('','','cbk','').br();
$ret.='<textarea id="codxt" class="console" style="min-width:550px; min-height:320px;">'.($d).'</textarea>';//htmlentities
