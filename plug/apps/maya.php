...
[$p,$o]=prmp($prm,$p,$o);
$dt=new DateTimeImmutable($p?$p:'now');
...
$ret=self::call('');
//return spanp($ret,['class'=>'txtx','title'=>'calendrier maya']);
return spanp($ret,['txtx','','','calendrier maya']);}
...
$j=$rid.'_maya,call_'.$pid.'_3_'.$p.'_'.$o;
$j=$rid.','.$bid.'_maya,call0_'.$pid.'_json_'.$p.'_'.$o;
$ret=inpdate($pid,$p,'','','',$j).lj('',$j,picto('ok')).' ';
$j=$rid.'_maya,call2_'.$bid.'_3_'.$p.'_'.$o;
