...
spl_autoload_register(function($a){$dr='progb/'; $r=['_','a','b','c'];
for($i=0;$i<4;$i++)if(is_file($f=$dr.$r[$i].'/'.$a.'.php')){require($f); return;}
$r=scandir('plug'); foreach($r as $v)if(is_file($f='plug/'.$v.'/'.$a.'.php')){require($f); return;}});
...
//lib
class ses{static $r=[]; static $enc=''; static $local=0;}
function auth(){return 1;}
...
$n=str_replace([' ','í','?',"'",'"',',',';',':',',/','%','&','$','ß','#','_','+','(',')','[',']','!',"\n",'\r','\0','[\]','~','{','}','´','ª'],'',($n));
return strtr($n,'‡·‚„‰ÁËÈÍÎÏÌÓÔÒÚÛÙıˆ˘˙˚¸˝ˇ¿¡¬√ƒ«»… ÀÃÕŒœ—“”‘’÷Ÿ⁄€‹›',
...
function test_connection(){$f=$_SESSION['connect'];
if(is_file($f)){require($f); if(test_mysql())return 'database ok'; else return 'euh no';}}
...
elseif($_GET['connexion']=='sav'){write_file($_SESSION['connect'],connect_file());
...
$f='plug/admin/install.php'; if(!is_file($f))$ret=dl_needed_file($f);
...
//config
function config(){
require($_SESSION['connect']); $f='cnfg/_'.sql::$db.'_config.txt';//$_SESSION['qd']
$ut=ses::$enc=='utf-8'?1:'';
...
function save_database(){$ret='';
$f='progb/a/sqldb.php'; if(!is_file($f))$ret.=dl_needed_file($f);
$f='progb/a/sqlop.php'; if(!is_file($f))$ret.=dl_needed_file($f);
$ret.=sqldb::batchinstall();
return $ret;}
...
$goto='install.php?step='.get('step'); $ret='';
if(get('upd')=='program'){$ret=dl_software(); w_master_p();}
else{$r=['plug/tar/pcltar.lib.php','plug/tar/pclerror.lib.php','plug/tar/pcltrace.lib.php','progb/d/sqldb.php','progb/d/sqlop.php','plug/apps/tar.php','plug/apps/software.php','css/_global.css','css/public_design_2.css'];
	foreach($r as $k=>$v)if(!is_file($v) or get('redo'))$ret.=dl_needed_file($v);
	if(!is_file('prog/index.php'))
	$ret.=lk($goto.'&upd=program','now: download_program').br();}
mkdir_r('img'); mkdir_r('imgc'); mkdir_r('_datas');
