...
$_SESSION['phpv'] = substr(phpversion(), 0, 3); //8.2
...
{
	if ($v) $_SESSION[$d] = $v;
	return isset($_SESSION[$d]) ? $_SESSION[$d] : '';
}
function sesif($d, $v = '')
{
	return isset($_SESSION[$d]) ? $_SESSION[$d] : ses($d, $v);
}
function lk($k, $v)
{
	return '<a href="' . $k . '">' . $v . '</a>';
}
function balise($l, $p, $t)
{
	$end = $t ? $t . '</' . $l . '>' : '';
	return '<' . $l . ' ' . $p . '>' . $end . "\n";
}
function divc($c, $v)
{
	return '<div' . atc($c) . '>' . $v . '</div>';
}
function write_file($f, $t)
{
	$h = fopen($f, 'w+');
	fwrite($h, $t);
	fclose($h);
...
{
	$d = $_SERVER['HTTP_HOST'];
	$d = substr($d, strrpos($d, '/'));
	return substr($d, 0, strpos($d, '.'));
}
function instlink($k, $v)
{
	return lk('?inst=' . $k . '&' . $k . '=' . $v, $k . '=' . $v) . br();
}
//lib
class ses
{
...
function make_form($r, $go)
{
	...
	foreach ($r as $k => $v) {
		if ($v == 'text' or $v == 'password') {
			$ret .= balise('input', 'type="' . $v . '" name="' . $k . '" id="' . $k . '" value="' . ($k == 'hub' ? dom() : $k) . '" size="44" maxlenght="255"', '');
		}
		if ($v == 'node') {
...
function connexion()
{
	...
	if ($_GET['connexion'] == 'verif_connexion') {
		$ok = test_connection();
		return $ok ? $ok : 'no connexion to mysql';
	} elseif ($_GET['connexion'] == 'sav') {
		write_file($_SESSION['connect'], connect_file());
...
{
	require($_SESSION['connect']);
	$q = mysqli_query(sql::$qr, 'select id from ' . ses('qd') . '_users where id=1');
...
{
	require($_SESSION['connect']);
	$f = 'cnfg/_' . sql::$db . '_config.txt'; //$_SESSION['qd']
	$ut = ses::$enc == 'utf-8' ? 1 : '';
...
function dl_needed_file($f)
{
	...
	if (!is_file($f)) {
		mkdir_r($dr);
		$u = $_SESSION['philum'] . '/call/software,give/' . $fu;
		$d = read_file($u);
...
function dl_prog()
{
	...
	$goto = 'install.php?step=' . get('step');
	$ret = '';
	if (get('upd') == 'program') {
		$ret = dl_software();
		w_master_p();
	} else {
		$r = ['plug/tar/pcltar.lib.php', 'plug/tar/pclerror.lib.php', 'plug/tar/pcltrace.lib.php', 'plug/install.php', 'plug/apps/tar.php', 'plug/apps/software.php', 'css/_global.css', 'css/public_design_2.css'];
...
function current_step($mn)
{
	...
	switch ($stpmn) {
		case ('connexion'):
			if (!$ok) $ret = instlink('connexion', '=');
			else $ret = $c;
			break;
		case ('databases'):
			if (!test_mysql()) $ret = instlink('install_databases', '=');
			else $ret = $c;
			break;
		case ('program'):
			$ret = dl_prog();
